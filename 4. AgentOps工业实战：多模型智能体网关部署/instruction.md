# 《现代软件开发技术》上机实验手册：多模型智能体网关部署实战，以Openclaw为例
...
| 时间段 | 教学环节 | 核心目标 | 关键技术栈 |
| :--- | :--- | :--- | :--- |
| **0-40'** | **模块一：实验前置准备与 OpenClaw 安装** | 使用官方脚本安装 OpenClaw，配置火山引擎。 | 官方安装脚本, Ark Helper, 火山引擎 |
| **40-80'** | **模块二：网关启动与架构体验** | 启动 OpenClaw 网关，体验默认配置的便捷性。 | OpenClaw Gateway, Debug 模式 |
| **80-120'** | **模块三：接入云端智能体** | 配置火山引擎豆包模型，完成本地到云端链路验证。 | Volcengine Ark, Coding Plan |
| **120-160'** | **模块四：飞书 WebSocket 配置与机器人权限** | 配置飞书通道，建立双向隧道，设置最小特权。 | Feishu, WebSocket, OAuth |
| **160-200'** | **模块五：本地模型接入与路由配置** | 接入 Ollama 本地模型，配置路由规则。 | Ollama, Router |
| **200-240'** | **模块六：端到端验收与总结** | 完成端到端测试，理解 IaC 理念。 | 端到端测试, 基础设施即代码 |
...
**👨‍🏫 核心思考**：请在实验报告中思考，`pip` 安装的包放置在 `.venv` 目录中，而 `ollama run` 下载的文件去哪了？它们在系统资源隔离级别上有什么本质不同？
