...
| **210-240'** | **阶段八：IaC 落盘验证与总结** | 查看生成的配置文件，理解声明式配置与可迁移部署。 | YAML, Config as Code |
...
1. 实验报告文档（Markdown、Word 或 PDF 均可）
2. 关键操作截图：网关启动、Provider/Agent/Channel/Router 配置、飞书端到端验收、`config.yaml` 片段
3. 配置说明文档：说明云端 Agent、本地 Agent、飞书 Channel、Router 前缀的对应关系
4. 问题记录：列出实验中遇到的至少 1 个问题及解决过程

### 2. 思考问题

1. `pip install` 安装的 Python 包与 `ollama run` 下载的模型文件在隔离级别上有什么不同？
2. 火山引擎配置中为什么使用 Endpoint ID，而不是直接填写模型名称？
3. 飞书机器人为什么应该使用最小权限配置？
4. `config.yaml` 如何体现基础设施即代码思想？

### 3. 评分说明

详细评分标准见同目录下的 `实验报告提交及评分标准.md`。
