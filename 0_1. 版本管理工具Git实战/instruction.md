...
在本次短学期实习中，我们选择Gitee为例子。

首先我们需要注册Gitee账号。然后进入 `个人主页 -> 个人设置 -> 邮箱管理` 选项中。

**作为重要的第一步，我们需要配置`提交邮箱`。**
...
在我们创建的文件上面，点击右键，选择`Git -> Commit File`。
...
然后，我们发起一个`Pull Request（合并请求）`，在Gitee中也称为`Merge Request`，告诉并请求管理者请合并version0的代码到master主线。
...
然后在评审的界面中，我们发现了这个合并产生了冲突（注意`存在冲突`红字）。
